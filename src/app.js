...
function getStopByName(title) {
  var stop;
  stops.forEach(function(element,index,array) {
   // console.log(routes[index].long_name + " - " + title);
    if(stops[index].name === title) {
      stop = stops[index];
    }
  });
  return stop;
...
function createMenu2(route, items) {
  menu2 = new UI.Menu({
      sections: [{
...
function routeSelect(e) {
  ...
  if(route !== undefined) {
    ...
    createMenu2(route,items);
  }
}

function stopSelect(e) {
  var rid = curRouteStop.route_id;
  var stop = getStopByName(e.item.title);
  console.log('https://transloc-api-1-2.p.mashape.com/arrival-estimates.json?agencies=643&callback=call&routes=' + rid + '&stops=' + stop.stop_id);
  showStop(rid,stop);
}
